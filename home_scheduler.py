...
# SQLITE3 DATABASE (bb_nvda_drops_data)
engine = sqlite3.connect("C:/Users/Julien/PycharmProjects/practice/data/" + 'bb_nvda_drops_data.db')
...
def get_html_from_page(url,):
    def grab_soup(url_, browser="firefox"):
        ...
        if browser == 'chrome':
            ...
            driver = webdriver.Chrome("C:/Users/Julien/PycharmProjects/practice/chromedriver95", options=chromeOptions)
        else:
...
def verify_if_new_drop_date():
    def sendemail_(TEXT, HTML):
        ...
        petrits_email = "petrit.ism@gmail.com"
    ...
    if next_date > prev_date:
        print("NEW DROP DATE")
        print(next_date)
        sendemail_(f'NEW DROP DATE: {next_date}. It is {next_date.strftime("%B %d, %Y")}. Queried: {parser.parse(query_date)} ET. Website: https://blog.bestbuy.ca/best-buy/nvidia',
                   wrap_in_paragraphs(f'WOO NEW DROP DATE: {next_date}. <br><br>It is {next_date.strftime("%B %d, %Y")}.<br><br>Queried: {parser.parse(query_date)} ET. <br><br>Website:<a href="https://blog.bestbuy.ca/best-buy/nvidia> https://blog.bestbuy.ca/best-buy/nvidia </a>"'))
    else:
        print("NOT A NEW DROP DATE")
        print(prev_date, next_date)

@dagster.op
def bb_nvda_func(context, cereals=None):
    verify_if_new_drop_date()
@dagster.op
def webscrap_bb_nvda_func():
    get_newest_best_buy_page()

@dagster.job
def bb_nvda_job():
    bb_nvda_func(webscrap_bb_nvda_func())

bb_nvda_job_schedule = dagster.ScheduleDefinition(cron_schedule="0 */3 * * *", job=bb_nvda_job, execution_timezone="US/Eastern", )

@dagster.repository
def home_repository():
    return [bb_nvda_job_schedule, bb_nvda_job, ]
