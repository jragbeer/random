...
# question_1 = question_1.set_index(['Year', "Sector"]).stack().reset_index()
# question_1.columns = ['Year', "Sector", "Feature", "Value"]
# question_1['Value'] = (np.round(question_1['Value'] , 0)).astype(int)
...
question_2 = fdf.sort_values("last_financing_deal_type", ascending=True)
ww = []
...
active_investors_count_mean=pd.NamedAgg(column="active_investors_count", aggfunc="mean"),

).reset_index().sort_values(['location_country',])
print(question_2.to_string())
question_2.to_csv(data_path + "marsdd_question_2a.csv", index = False, )
...
question_4 = fdf.drop_duplicates(subset=['company', 'total_raised_cad']).sort_values("location_hq", ascending=True)
...
# QUESTION 5
# number of active investors per company
# print(fdf['active_investors_count'].value_counts())
